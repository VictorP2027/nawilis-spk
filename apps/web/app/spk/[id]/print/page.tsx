  ...
  return (
    <div className="sheet-wrap print-wrap">
      ...
      <div className="sheet">
        ...
        {/* The body diagram prints ALWAYS — the paper-faithful art with the
            operator's ink laid back over it in perfect register; a pristine
            car prints it blank, ready for a pen. Old documents that only
            recorded tapped zones list them as text below. */}
        {(() => {
          const oldZones = doc.rawForm?.kerusakan_zones ?? [];
          const ink = doc.damageDiagram?.imageRef;
          return (
            <div className="box">
              <span className="sec-h">KERUSAKAN BODI{ink ? '' : ' — coret manual bila perlu'}</span>
              <div style={{ position: 'relative', margin: '0 auto', width: 168 }}>
                <CarDiagram />
                {ink?.startsWith('data:image') && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={ink} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} />
                )}
              </div>
              {oldZones.length > 0 && (
                <div style={{ fontSize: 10, marginTop: 2 }}>
                  Ditandai: {oldZones.map((c) => DAMAGE_ZONES.find((z) => z.code === c)?.label ?? c).join(', ')}
                </div>
              )}
            </div>
          );
